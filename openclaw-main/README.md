...
## Quick Start: Google Chat Finance Bot

- Guide: [docs/googlechat-finance.md](docs/googlechat-finance.md)
- Set env: `GOOGLE_CHAT_PROJECT_ID`, `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON` or `GOOGLE_CHAT_SERVICE_ACCOUNT_PATH`, `GOOGLE_CHAT_SPACE_ALLOWLIST`, plus `GOOGLE_CHAT_AUDIENCE_TYPE` if using app-url.
- Apply the FinanceGroup config (mention-only, space-allowlisted, safe tools: no exec/fs/browser) and bind it to your Google Chat space.
- Run locally: `pnpm install && pnpm openclaw gateway --port 18789 --verbose`, then add the bot to the target space and @mention it to verify.
